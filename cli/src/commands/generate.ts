...
async function generateBatchImages(options: BatchGenerationOptions) {
  ...
  for (let i = 0; i < prompts.length; i += maxConcurrent) {
    ...
    const batchPromises = batch.map(async (prompt, batchIndex) => {
      ...
      const spinner = ora(
        `Processing prompt ${globalIndex + 1}/${prompts.length}: ${prompt.substring(0, 50)}...`,
      ).start();
      ...
      try {
        ...
        if (result.success && result.data?.data[0]?.b64_json) {
          ...
          console.log(
            chalk.green(`✅ ${globalIndex + 1}/${prompts.length}: ${filename}`),
          );
          return { success: true, prompt, filename };
        } else {
          console.log(
            chalk.red(
              `❌ ${globalIndex + 1}/${prompts.length}: ${result.error}`,
            ),
          );
        ...
        spinner.stop();
        console.log(
          chalk.red(`❌ ${globalIndex + 1}/${prompts.length}: ${error}`),
        );
        return {
          success: false,
          prompt,
