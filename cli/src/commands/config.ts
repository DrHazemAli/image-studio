  ...
  configCommand
    ...
    .action(async (options) => {
      try {
        ...
        console.log(chalk.blue('Initializing Azure Image Studio configuration...'));
        await configManager.createDefaultConfig();
        
        console.log(chalk.green('✅ Configuration initialized successfully!'));
        console.log(chalk.yellow('📝 Please edit src/app/config/azure-config.json with your Azure endpoints'));
        console.log(chalk.yellow('🔑 Set your API key with: azure-image-studio config set-api-key'));
    ...
    .action(async (options) => {
      try {
        ...
        const displayConfig = {
          ...
          apiKey: options.hideSensitive ? '[HIDDEN]' : (apiKey ? `${apiKey.substring(0, 8)}...` : '[NOT SET]'),
          cliConfig: cliConfig
        };
  ...
  configCommand
    ...
    .action(async (options) => {
      try {
        ...
        const modelId = options.model || config.endpoints[0]?.deployments[0]?.id;
        if (!modelId) {
          console.log(chalk.red('❌ No model specified and no default model found'));
          process.exit(1);
        }
