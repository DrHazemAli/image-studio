...
export function createProjectCommand(): Command {
  ...
  projectCommand
    .description('Manage Azure Image Studio projects')
    .addHelpText('after', `
Examples:
  ...
  projectCommand
    ...
    .option('--template <template>', 'Project template (blank, social-media, product, art)')
    .action(async (options) => {
      try {
        ...
        await fs.writeJson(path.join(projectDir, 'project.json'), projectMetadata, { spaces: 2 });
    ...
    .action(async (options) => {
      try {
        ...
        if (projectDirs.length === 0) {
          console.log(chalk.yellow('No projects found. Create a project with: azure-image-studio project create'));
          return;
        }
        ...
        for (const projectDir of projectDirs) {
          ...
          if (await fs.pathExists(projectJsonPath)) {
            try {
              ...
              console.log(chalk.yellow(`⚠️  Skipping invalid project: ${projectDir}`));
            }
          }
        }
  ...
  projectCommand
    ...
    .action(async (options) => {
      try {
          ...
          const answers = await inquirer.prompt([
            {
              ...
              choices: projects.map(p => ({ name: `${p.name} (${p.template})`, value: p.id }))
            }
          ]);
        ...
        const exportPath = path.join(outputDir, `${exportName}.${options.format}`);
    ...
    .action(async (options) => {
      try {
        ...
        try {
          ...
          } else if (importFile.endsWith('.tar') || importFile.endsWith('.tar.gz')) {
          ...
          if (await fs.pathExists(projectDir) && !options.overwrite) {
            console.log(chalk.red(`❌ Project "${projectData.name}" already exists. Use --overwrite to replace it.`));
            return;
          }
          ...
          console.log(chalk.green(`✅ Project "${projectData.name}" imported successfully!`));
          console.log(chalk.blue(`📁 Project directory: ${projectDir}`));
  ...
  projectCommand
    ...
    .action(async (options) => {
      try {
        ...
        await fs.remove(projectDir);
        console.log(chalk.green(`✅ Project "${projectData.name}" deleted successfully!`));
...
async function createBlankTemplate(templatesDir: string) {
  ...
  await fs.writeJson(path.join(templatesDir, 'template.json'), templateData, { spaces: 2 });
}
...
async function createZipExport(projectDir: string, exportPath: string, options: any) {
...
async function createTarExport(projectDir: string, exportPath: string, options: any) {
