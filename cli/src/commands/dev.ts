...
Examples:
  ...
  devCommand
    ...
    .action(async (options) => {
      try {
        console.log(chalk.blue('🚀 Starting Azure Image Studio development server...'));
        
        // Check if we're in the right directory
        const packageJsonPath = path.join(process.cwd(), 'package.json');
        if (!(await fs.pathExists(packageJsonPath))) {
          console.log(chalk.red('❌ Not in Azure Image Studio project directory.'));
          console.log(chalk.yellow('Please run this command from the project root directory.'));
          process.exit(1);
        }
  ...
  devCommand
    ...
    .action(async (options) => {
      try {
        ...
        if (!endpoint) {
          console.log(chalk.red('❌ Endpoint URL is required. Use --endpoint <url>'));
          process.exit(1);
        }

        if (!apiKey) {
          console.log(chalk.red('❌ API key is required. Use --key <key> or set AZURE_API_KEY'));
          process.exit(1);
        }
        ...
        try {
          ...
          const response = await fetch(`${endpoint}/openai/deployments?api-version=2025-04-01-preview`, {
            method: 'GET',
            headers: {
          ...
          if (response.ok) {
            ...
            try {
              ...
              if (data.data && Array.isArray(data.data)) {
                console.log(chalk.blue(`📋 Found ${data.data.length} deployments:`));
                data.data.forEach((deployment: any) => {
                  console.log(chalk.gray(`  • ${deployment.id} (${deployment.model})`));
                });
              }
            } catch (error) {
  ...
  devCommand
    ...
    .action(async (options) => {
      try {
        console.log(chalk.blue('🛠️  Setting up Azure Image Studio development environment...'));
        ...
        if (!options.skipConfig) {
          ...
          if (!apiKey) {
            console.log(chalk.yellow('⚠️  No API key found. You can set one with:'));
        ...
        console.log(chalk.gray('1. Configure your Azure endpoints in src/app/config/azure-config.json'));
        console.log(chalk.gray('2. Set your API key: azure-image-studio config set-api-key'));
        console.log(chalk.gray('3. Start the development server: azure-image-studio dev start'));
        
      } catch (error) {
        console.error(chalk.red('❌ Failed to setup development environment:'), error);
        process.exit(1);
      }
    });
    ...
    .action(async (options) => {
      try {
        ...
        if (!(await fs.pathExists(logFile))) {
          console.log(chalk.yellow('No log file found. Start the development server to generate logs.'));
          return;
        }
