  ...
  modelsCommand
    ...
    .action(async (options) => {
      try {
        ...
        if (!config) {
          console.log(chalk.yellow('No configuration found. Run "azure-image-studio config init" to create one.'));
          return;
        }
        ...
        console.log(boxen(table(tableData, {
          header: {
            alignment: 'center',
        ...
        if (options.verbose) {
          ...
          config.endpoints.forEach((endpoint, index) => {
            ...
            console.log(chalk.gray(`   Models: ${endpoint.deployments.length}`));
          });
        }
    ...
    .action(async (options) => {
      try {
        ...
        if (!deploymentInfo) {
          ...
          console.log(chalk.yellow('Run "azure-image-studio models list" to see available models.'));
          return;
        }
        ...
        console.log(boxen(
          Object.entries(modelInfo)
            .map(([key, value]) => `${chalk.blue(key)}: ${chalk.white(value)}`)
            .join('\n'),
          {
  ...
  modelsCommand
    ...
    .action(async (options) => {
      try {
        ...
        if (!config || !apiKey) {
          console.log(chalk.red('❌ Configuration or API key not found. Run "azure-image-studio config init" first.'));
          return;
        }
        ...
        console.log(chalk.blue(`🧪 Testing model: ${deploymentInfo.deployment.name}`));
        console.log(chalk.gray(`Prompt: ${options.prompt}`));

        const spinner = ora('Testing model...').start();

        try {
          const result = await provider.generateImage(options.model, {
          ...
          if (result.success) {
            ...
            console.log(chalk.green(`Generated image with ${result.data?.data.length || 0} result(s)`));
  ...
  modelsCommand
    ...
    .action(async () => {
      try {
        ...
        if (!apiKey) {
          console.log(chalk.red('❌ API key not found. Run "azure-image-studio config set-api-key" first.'));
          return;
        }
