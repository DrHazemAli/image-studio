  ...
  try {
    ...
    try {
      const response = await fetch(
        `${catalogEndpoint}?type=${modelType}${provider ? `&provider=${provider}` : ""}`,
        {
          headers: {
      ...
      const transformedModels =
        catalogData.models?.map((model: any) => ({
          ...
          description:
            model.description || `${model.name} model from Azure Model Catalog`,
      ...
      console.warn(
        "Failed to fetch from Azure Model Catalog, returning fallback data:",
        fetchError,
      );
      ...
      return NextResponse.json({
        ...
        source: "fallback",
        message:
          "Azure Model Catalog unavailable, using fallback model templates",
      });
    }
  } catch (error) {
    ...
    openai: [
      {
        ...
        name: "DALL-E 3",
        description:
          "High-quality image generation with natural and vivid styles",
    ...
    blackForestLabs: [
      {
        ...
        supportedSizes: [
          ...
          {
            size: "2752x1536",
            label: "Ultra Landscape (16:9)",
            aspect: "16:9",
            description: "Ultra HD landscape",
          },
        ],
        supportedFormats: ["png", "jpeg"],
        maxTokens: 5000,
      ...
      },
      {
        ...
        description:
          "Context-aware image generation and editing with image input support",
        provider: "Black Forest Labs",
        apiVersion: "2024-12-01",
        capabilities: [
          "text-to-image",
        ...
        supportedSizes: [
          ...
          {
            size: "2048x2048",
            label: "Ultra Square (1:1)",
            aspect: "1:1",
            description: "Ultra HD square",
          },
        ],
        supportedFormats: ["png", "jpeg"],
        maxTokens: 5000,
    ...
    microsoft: [
      {
        ...
        name: "Florence 2.0",
        description:
          "Microsoft's advanced vision-language model for image understanding and generation",
