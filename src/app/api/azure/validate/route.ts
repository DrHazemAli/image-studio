...
export async function POST(request: NextRequest) {
  try {
    ...
    const baseUrl = endpoint || getEnvVar('AZURE_API_BASE_URL', 'https://api.openai.com');
    ...
    if (response.ok) {
      ...
      if (processedBaseUrl.includes('services.ai.azure.com') && response.status === 404) {
      ...
      return NextResponse.json({
        valid: false,
        message: errorData.error?.message || errorData.message || `Azure API validation failed (${response.status})`,
