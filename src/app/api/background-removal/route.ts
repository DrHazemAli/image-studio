...
export async function POST(request: NextRequest) {
  try {
    ...
    if (modelConfig.requiresApproval) {
      ...
      console.warn(`Model '${requestedModel}' requires approval but proceeding for demo`);
    }
    ...
    const backgroundRemovalRequest: BackgroundRemovalRequest = {
      ...
      edgeRefinement: edgeRefinement !== undefined ? edgeRefinement : bgRemovalConfig.defaultSettings.edgeRefinement,
      transparencyMode: transparencyMode || bgRemovalConfig.defaultSettings.transparencyMode,
      output_format: outputFormat || bgRemovalConfig.defaultSettings.outputFormat
    };
