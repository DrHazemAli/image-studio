...
export async function POST(request: NextRequest) {
  try {
    ...
    const searchParams: AssetSearchParams = {
      ...
      page,
      perPage,
      sortBy: sortBy === 'relevant' || sortBy === 'latest' || sortBy === 'popular' 
        ? sortBy 
        : 'relevant',
    };

    const assetTypeEnum = assetType as AssetType;
    const results = await assetManager.searchAssets(searchParams, assetTypeEnum);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const assetTypeEnum = assetType as AssetType;
    const results = await assetManager.searchAssets(searchParamsObj, assetTypeEnum);
