          ...
          if (response.ok) {
            ...
            if (data.config) {
              ...
              const configWithValidation = {
                ...data.config,
                primary: {
                  status: (data.config.primary?.status || "idle") as
                    | "idle"
                    | "valid"
                    | "invalid"
                    | "pending",
                ...
                endpoints:
                  data.config.endpoints?.map((endpoint: AzureEndpoint) => ({
                    ...endpoint,
                    status: (endpoint.status || "idle") as
                      | "idle"
                      | "valid"
                      | "invalid"
                      | "pending",
    ...
    try {
      ...
      if (data.valid) {
        ...
        try {
          ...
          logger.info(
            "Saving primary validation with config:",
        ...
        try {
          ...
          logger.info(
            "Saving primary validation error with config:",
            JSON.stringify(updatedConfig, null, 2),
          );
  ...
  const validateEndpoint = async (endpoint: AzureEndpoint) => {
    ...
    if (!apiKey || !baseUrl) {
      return {
        valid: false,
        message:
          "API key and endpoint are required (use primary values or set endpoint-specific values)",
      };
    }
    ...
    setAzureConfig((prev) => {
      ...
      logger.info(
        "Saving pending status with config:",
        JSON.stringify(updatedConfig, null, 2),
      );
    ...
    try {
      ...
      setAzureConfig((prev) => {
        ...
        const currentEndpoint =
          prev.endpoints.find((ep) => ep.id === endpoint.id) || endpoint;
        ...
        logger.info(
          "Saving final validation with config:",
          JSON.stringify(updatedConfig, null, 2),
        );
        ...
        fetch("/api/azure/config", {
          ...
          .catch((saveError) => {
            logger.warn(
              "Failed to save validation status to server:",
              saveError,
            );
          });
      ...
      setAzureConfig((prev) => {
        ...
        logger.info(
          "Saving error status with config:",
          JSON.stringify(updatedConfig, null, 2),
        );
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Primary API key used as fallback when endpoint-specific keys are
                not provided.
                <br />
                <span className="text-gray-500 dark:text-gray-400">
                  If you have AZURE_API_KEY in your environment variables, it
                  will be automatically loaded as the fallback key.
                </span>
              </p>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                Primary endpoint URL used as fallback when endpoint-specific
                URLs are not provided.
                <br />
                <span className="text-gray-500 dark:text-gray-400">
                  If you have AZURE_ENDPOINT in your environment variables, it
                  will be automatically loaded as the fallback endpoint.
                </span>
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            {endpoint.validated_at && (
              <p className="text-xs text-gray-400 dark:text-gray-500">
                Last validated:{" "}
                {new Date(endpoint.validated_at).toLocaleString()}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="mt-4 space-y-4 pt-4 border-t border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                This is the actual deployment name used by Azure OpenAI. It must
                match the deployment name in your Azure resource.
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
              ...
              <input
                ...
                placeholder={
                  primaryEndpoint || "https://your-endpoint.openai.azure.com"
                }
                className="w-full px-3 py-2 rounded-lg border border-gray-200/50 dark:border-gray-700/50 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/50 focus:border-transparent transition-colors duration-150"
              />
              {!endpoint.baseUrl && primaryEndpoint && (
            ...
            <div>
              ...
              {!endpoint.apiKey && primaryApiKey ? (
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  Specific API key for this endpoint. If empty, primary API key
                  will be used.
                </p>
              )}
            </div>
