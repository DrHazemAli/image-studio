...
export function AssetStoreSettings({ onUpdate }: AssetStoreSettingsProps) {
  ...
  const ensureConfigStructure = useCallback(
    (config: unknown): AssetStoreConfig => {
      ...
      return {
        ...
        providers: {
          unsplash: {
            enabled: Boolean(
              (providers.unsplash as Record<string, unknown>)?.enabled ?? false,
            ),
            apiKey: "", // API keys are never stored in localStorage, always empty
            rateLimit: Number(
              (providers.unsplash as Record<string, unknown>)?.rateLimit ?? 50,
            ),
          },
          pexels: {
            enabled: Boolean(
              (providers.pexels as Record<string, unknown>)?.enabled ?? false,
            ),
            apiKey: "", // API keys are never stored in localStorage, always empty
            rateLimit: Number(
              (providers.pexels as Record<string, unknown>)?.rateLimit ?? 200,
            ),
          },
        },
        ui: {
    ...
    setConfig((prev) => ({
      ...
      providers: {
        ...
        unsplash: {
          ...
          apiKey: unsplashKey
            ? decodeURIComponent(unsplashKey)
            : prev.providers?.unsplash?.apiKey || "",
        },
        pexels: {
          ...prev.providers?.pexels,
          apiKey: pexelsKey
            ? decodeURIComponent(pexelsKey)
            : prev.providers?.pexels?.apiKey || "",
        },
      },
    }));
  }, []);
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        ...
        const savedConfig = getUnifiedSetting(
          "assetStore",
          defaultConfig,
          "localStorage",
        ) as AssetStoreConfig;
  ...
  const saveConfig = useCallback(
    async (newConfig: AssetStoreConfig) => {
  ...
  const validateApiKey = useCallback(
    async (provider: "unsplash" | "pexels") => {
  ...
  const handleApiKeyChange = useCallback(
    (provider: "unsplash" | "pexels", value: string) => {
      setConfig((prev) => ({
        ...prev,
        providers: {
          ...prev.providers,
      ...
      <div className="flex items-center justify-center p-8">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Loading asset store configuration...
          </p>
        </div>
      </div>
    );
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <button
                    onClick={() =>
                      validateApiKey(selectedProvider as "unsplash" | "pexels")
                    }
                    disabled={
              ...
              <ProviderCard
                ...
                config={{
                  enabled:
                    config.providers[
                      selectedProvider as keyof typeof config.providers
                    ]?.enabled ?? false,
                  apiKey:
                    config.providers[
                      selectedProvider as keyof typeof config.providers
                ...
                onToggleApiKeyVisibility={() =>
                  toggleApiKeyVisibility(
                    selectedProvider as "unsplash" | "pexels",
                  )
                }
                onUpdate={(updates) => {
            ...
            <div className="p-4 rounded-lg bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateSetting("ui.defaultView", e.target.value)
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateSetting("ui.itemsPerPage", parseInt(e.target.value))
                    }
                    className="w-full px-3 py-2 rounded-lg border border-gray-200/50 dark:border-gray-700/50 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/50 focus:border-transparent transition-colors duration-150"
                  />
                </div>
              ...
              <div className="flex items-center gap-2 mt-4">
                <input
                  ...
                  onChange={(e) =>
                    updateSetting("ui.showAttribution", e.target.checked)
                  }
                  className="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">
            ...
            <div className="p-4 rounded-lg bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50">
              <div className="flex items-center gap-2 mb-4">
                <input
                  ...
                  onChange={(e) =>
                    updateSetting("cache.enabled", e.target.checked)
                  }
                  className="rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700 dark:text-gray-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateSetting("cache.ttl", parseInt(e.target.value))
                      }
                      className="w-full px-3 py-2 rounded-lg border border-gray-200/50 dark:border-gray-700/50 bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500/50 focus:border-transparent transition-colors duration-150"
                    />
                  </div>
          ...
          <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm text-blue-800 dark:text-blue-200">
                ...
                <p>
                  Your API keys are stored securely in browser cookies and are
                  never sent to our servers. They are only used to make direct
                  requests to the respective APIs from your browser.
                </p>
              </div>
            </div>
          </div>
    ...
    <div className="p-4 rounded-lg bg-white/50 dark:bg-gray-800/50 backdrop-blur-sm border border-gray-200/50 dark:border-gray-700/50">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div
            className={`w-12 h-12 rounded-lg bg-gradient-to-br ${getProviderColor()} flex items-center justify-center`}
          >
            {getProviderIcon()}
          </div>
          <div>
            <div className="flex items-center gap-2">
              ...
              <span
                className={`px-2 py-1 text-xs font-medium bg-${info.color}-100 dark:bg-${info.color}-900/50 text-${info.color}-600 dark:text-${info.color}-400 rounded-full`}
              >
                Free
              </span>
              {safeConfig.enabled && (
