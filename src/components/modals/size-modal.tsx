...
export const SizeModal = React.memo<SizeModalProps>(({ 
  ...
  const modelName = useMemo(() => getModelName(currentModel), [getModelName, currentModel]);
  ...
  return (
    <BaseModal
      ...
      icon={<AspectRatioIcon className="w-5 h-5 text-blue-600 dark:text-blue-400" />}
      size="lg"
    >
        ...
        <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4">
          ...
          <ul className="text-sm text-gray-600 dark:text-gray-400 space-y-2">
            <li className="flex items-start gap-2">
              ...
              <div>
                <strong>Square (1:1)</strong> - Best for social media, avatars, and balanced compositions
              </div>
            </li>
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 bg-green-500 rounded-full mt-2 flex-shrink-0" />
              <div>
                <strong>Portrait (2:3, 3:4)</strong> - Ideal for mobile screens and vertical content
              </div>
            </li>
            <li className="flex items-start gap-2">
              <span className="w-1.5 h-1.5 bg-purple-500 rounded-full mt-2 flex-shrink-0" />
              <div>
                <strong>Landscape (3:2, 16:9)</strong> - Perfect for headers, banners, and wide scenes
              </div>
            </li>
            {currentModel.includes('flux') && (
              <li className="flex items-start gap-2">
                ...
                <div>
                  <strong>Ultra sizes</strong> - High resolution for professional use (slower generation)
                </div>
              </li>
            )}
          </ul>
