...
export function GlassyPrompt({ isOpen, onClose, onGenerate, isGenerating = false, progress = 0 }: GlassyPromptProps) {
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Select.Root value={size} onValueChange={setSize}>
                    <Select.Trigger className="flex items-center gap-2 bg-gray-800/50 border border-gray-600/30 rounded-full px-4 py-2 text-white hover:bg-gray-700/50 transition-colors">
                      ...
                      <span className="text-sm">{size === '1024x1024' ? '2:3' : size === '1792x1024' ? '16:9' : '3:2'}</span>
                      <ChevronDownIcon className="w-3 h-3" />
                    </Select.Trigger>
                    <Select.Portal>
                      <Select.Content className="bg-gray-900/95 backdrop-blur-xl border border-gray-600/50 rounded-xl shadow-2xl p-2 z-50">
                        <Select.Viewport>
                          <Select.Item value="1024x1792" className="p-3 rounded-lg hover:bg-gray-800/50 cursor-pointer text-white">
                            <div className="flex items-center gap-3">
                              <div className="w-3 h-4 border border-gray-400 rounded-sm" />
                              <span>2:3 (Portrait)</span>
                            </div>
                          </Select.Item>
                          <Select.Item value="1024x1024" className="p-3 rounded-lg hover:bg-gray-800/50 cursor-pointer text-white">
                            <div className="flex items-center gap-3">
                              <div className="w-4 h-4 border border-gray-400 rounded-sm" />
                              <span>1:1 (Square)</span>
                            </div>
                          </Select.Item>
                          <Select.Item value="1792x1024" className="p-3 rounded-lg hover:bg-gray-800/50 cursor-pointer text-white">
                            <div className="flex items-center gap-3">
            ...
            <div className="p-6 space-y-6">
                    ...
                    <div className="flex items-center justify-between w-full">
                      <div className="flex items-center gap-2">
                        ...
                        {selectedModelData?.premium && (
                          <Badge size="1" className="bg-gradient-to-r from-yellow-400 to-orange-500 text-white">
                            Pro
                          </Badge>
                        )}
                      </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                  ...
                  <Select.Root value={size} onValueChange={setSize}>
                    ...
                    <Select.Portal>
                      <Select.Content className="bg-white/90 dark:bg-black/90 backdrop-blur-xl border border-white/20 dark:border-white/10 rounded-lg shadow-xl p-1">
                        <Select.Viewport>
                          <Select.Item value="1024x1024" className="p-2 hover:bg-white/20 dark:hover:bg-white/10 rounded text-sm text-gray-900 dark:text-white">
                            Square (1024×1024)
                          </Select.Item>
                          <Select.Item value="1792x1024" className="p-2 hover:bg-white/20 dark:hover:bg-white/10 rounded text-sm text-gray-900 dark:text-white">
                            Landscape (1792×1024)
                          </Select.Item>
                          <Select.Item value="1024x1792" className="p-2 hover:bg-white/20 dark:hover:bg-white/10 rounded text-sm text-gray-900 dark:text-white">
                            Portrait (1024×1792)
                          </Select.Item>
                        </Select.Viewport>
                      </Select.Content>
                ...
                <div>
                  ...
                  <Select.Root value={quality} onValueChange={setQuality}>
                    ...
                    <Select.Portal>
                      <Select.Content className="bg-white/90 dark:bg-black/90 backdrop-blur-xl border border-white/20 dark:border-white/10 rounded-lg shadow-xl p-1">
                        <Select.Viewport>
                          <Select.Item value="standard" className="p-2 hover:bg-white/20 dark:hover:bg-white/10 rounded text-sm text-gray-900 dark:text-white">
                            Standard
                          </Select.Item>
                          <Select.Item value="hd" className="p-2 hover:bg-white/20 dark:hover:bg-white/10 rounded text-sm text-gray-900 dark:text-white">
                            HD
                          </Select.Item>
                        </Select.Viewport>
                      </Select.Content>
                ...
                <div>
                  ...
                  <Select.Root value={count.toString()} onValueChange={(v) => setCount(Number(v))}>
                    <Select.Trigger className="w-full p-2.5 bg-white/20 dark:bg-white/5 border border-white/30 dark:border-white/10 rounded-lg text-sm backdrop-blur-sm text-gray-900 dark:text-white">
                      <Select.Value />
                      <Select.Icon><ChevronDownIcon /></Select.Icon>
                    </Select.Trigger>
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400 text-center bg-white/5 dark:bg-white/5 rounded-lg p-2 backdrop-blur-sm">
                Press <kbd className="px-1.5 py-0.5 bg-white/20 dark:bg-white/10 rounded text-xs">⌘ Enter</kbd> to generate • <kbd className="px-1.5 py-0.5 bg-white/20 dark:bg-white/10 rounded text-xs">Esc</kbd> to close
              </div>
            </div>
          </div>
        </motion.div>
