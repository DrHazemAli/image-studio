...
export function Toolbar({ activeTool, onToolChange }: ToolbarProps) {
  ...
  const handleDrag = (
    ...
    const newY = Math.max(
      0,
      Math.min(window.innerHeight - height - 100, info.point.y),
    );
  ...
  return (
    <Tooltip.Provider>
      ...
      >
        <div
          className={`studio-toolbar bg-white/90 dark:bg-black/90 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xl h-full flex flex-col transition-all duration-200 ${
        ...
        >
          ...
          <div className="flex-1 flex flex-col gap-1 p-2 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600 scrollbar-track-transparent">
            {tools.map((tool, index) => {
              ...
              const showSeparator =
                index > 0 && groupSeparators.has(tool.group);
