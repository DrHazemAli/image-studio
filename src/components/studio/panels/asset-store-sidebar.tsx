...
export function AssetStoreSidebar({
  ...
  const loadFeaturedAssets = useCallback(async (page = 1, append = false) => {
    if (page === 1) {
      setIsLoading(true);
      setError(null);
    } else {
  ...
  const searchAssets = useCallback(async (query?: string, page = 1, append = false) => {
  ...
  }, [activeTab, searchQuery, selectedCategory, currentPage, hasMore, isLoadingMore, searchAssets, loadFeaturedAssets]);
      ...
      {Array.from({ length: 6 }).map((_, index) => (
        <motion.div
          ...
          className={`bg-gray-200 dark:bg-gray-700 rounded-lg animate-pulse ${
            viewMode === "grid" ? "aspect-square" : "flex items-center gap-3 p-3 h-16"
          }`}
        >
          {viewMode === "grid" ? (
            ...
            {isOpen && (
              ...
              <div className="flex gap-1 p-3 border-b border-gray-200 dark:border-zinc-900 flex-shrink-0">
                  {[
                    ...
                    <button
                      key={tab.id}
                      onClick={() => setActiveTab(tab.id as "featured" | "search" | "categories")}
              ...
              <div className="flex items-center justify-between p-3 border-b border-gray-200 dark:border-zinc-900 flex-shrink-0">
                <div className="flex items-center gap-3">
                  <span className="text-xs text-gray-500 dark:text-gray-400">
                    {totalResults > 0 ? `${assets.length} of ${totalResults} assets` : `${assets.length} assets`}
                  </span>
                  {/* Filter Icon */}
