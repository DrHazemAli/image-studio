...
export const useRealTimePreview = (
   ...
   */
  const adjustmentsToCSSFilter = useCallback((adjustments: ImageAdjustments): string => {
    const filters = [];
   ...
   */
  const applyAdjustments = useCallback((adjustments: ImageAdjustments, immediate = false) => {
    if (!fabricCanvas || !selectedImage || !(selectedImage instanceof FabricImage)) {
      return;
    }
      ...
      try {
          ...
          if (canvasElement && ctx) {
            ...
            if (canvasFilterString) {
              ...
              try {
                ...
                console.log('Applied canvas context filter:', canvasFilterString);
                filtersApplied = true;
              } catch (ctxError) {
              ...
              if (!filtersApplied) {
                try {
                  ...
                  console.log('Applied canvas element filter:', canvasFilterString);
                  filtersApplied = true;
                } catch (elementError) {
              ...
              if (!filtersApplied) {
                try {
                  ...
                  if (imageElement) {
                    ...
                    if (tempCtx) {
                      tempCanvas.width = imageElement.width || selectedImage.width!;
                      tempCanvas.height = imageElement.height || selectedImage.height!;
                      ...
                      (selectedImage as FabricImage).setSrc(filteredImageData, () => {
                  ...
                  console.warn('Manual filter processing failed:', manualError);
                }
              }
            }
          }
        ...
        if (!filtersApplied) {
          ...
          if (imageElement && imageElement.style) {
            ...
            console.log('Applied CSS filters as fallback:', imageElement.style.filter);
          }
        }
  ...
  const resetAdjustments = useCallback(() => {
    ...
    try {
      ...
      if (originalImagePropsRef.current) {
        const imageElement = (selectedImage as FabricImage).getElement();
        if (imageElement && imageElement.style) {
          imageElement.style.filter = originalImagePropsRef.current.filters || '';
        }
      ...
      try {
        ...
        if (originalImagePropsRef.current && (selectedImage as FabricImage).getSrc) {
          try {
            ...
            if (currentSrc && currentSrc.startsWith('data:image/')) {
              console.log('Image may have been modified by filters, but no original stored');
