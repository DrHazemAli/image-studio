...
import { StoredImageAdjustments, ProjectImageAdjustments } from '../types/image-adjustments-storage';
...
export interface AdjustmentsPersistenceConfig {
  ...
  onSave?: (projectId: string, adjustments: ProjectImageAdjustments) => Promise<void>;
...
export const useAdjustmentsPersistence = (
  ...
  const generateImageId = useCallback((imageObject: FabricObject): string => {
    ...
    if (imageObject.id) {
      return imageObject.id.toString();
    }
   ...
   */
  const saveImageAdjustments = useCallback(async (
    ...
    try {
      ...
      const imageSrc = imageObject instanceof FabricImage ? imageObject.getSrc() : undefined;
  ...
  const loadImageAdjustments = useCallback((
    ...
    try {
      ...
      if (storedAdjustments) {
        console.log('Loaded adjustments for image:', imageId, storedAdjustments.adjustments);
   ...
   */
  const removeImageAdjustments = useCallback(async (imageObject: FabricObject) => {
    if (!projectId || !onSave) {
      return;
    }
  ...
  const loadProjectAdjustments = useCallback(async () => {
    ...
    try {
      ...
      if (projectData) {
        ...
        console.log('Loaded project adjustments:', projectId, Object.keys(projectData.images).length, 'images');
      }
    } catch (error) {
   ...
   */
  const getAllImageAdjustments = useCallback((): Record<string, StoredImageAdjustments> => {
   ...
   */
  const hasStoredAdjustments = useCallback((imageObject: FabricObject): boolean => {
  ...
  const applyStoredAdjustments = useCallback((
    applyFunction: (imageObject: FabricObject, adjustments: ImageAdjustments) => void
  ) => {
    ...
    objects.forEach(obj => {
      if (obj instanceof FabricImage) {
        ...
        const hasAdjustments = Object.entries(storedAdjustments).some(([key, value]) => {
