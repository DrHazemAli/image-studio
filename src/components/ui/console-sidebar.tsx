...
import { ChevronLeftIcon, ChevronRightIcon, CodeIcon } from '@radix-ui/react-icons';
import { Card, Box, Text, Flex, Button, ScrollArea, Badge } from '@radix-ui/themes';
...
export function ConsoleSidebar({ requestLog, responseLog, isOpen, onToggle }: ConsoleSidebarProps) {
        ...
        <Flex direction="column" height="100%">
          ...
          >
            <Flex align="center" gap="2">
              <CodeIcon className={resolvedTheme === 'dark' ? 'text-white' : 'text-gray-700'} />
              <Text 
                size="2" 
                weight="medium"
            ...
            <Button
              ...
              size="1"
              onClick={onToggle}
              className={resolvedTheme === 'dark' ? 'text-gray-300 hover:text-white' : 'text-gray-600 hover:text-gray-900'}
            >
                ...
                >
                  <div className="flex">
                    ...
                    >
                      ...
                      {responseLog?.status && typeof responseLog.status === 'number' ? (
                        <Badge 
                ...
                >
                  {activeTab === 'request' && requestLog && (
                    <Box>
                      ...
                      >
                        {requestLog.timestamp ? String(requestLog.timestamp) : 'No timestamp'}
                      </Text>
                  ...
                  {activeTab === 'response' && responseLog && (
                    <Box>
                      ...
                      >
                        {responseLog.timestamp ? String(responseLog.timestamp) : 'No timestamp'}
                      </Text>
                      <Box className="space-y-3">
                        {responseLog.status && typeof responseLog.status === 'number' ? (
                          <Box>
                            ...
                            >
                              {responseLog.statusText ? String(responseLog.statusText) : 'No status text'}
                            </Text>
                            </Flex>
                          </Box>
                        ) : null}
                        ...
                        {responseLog.error && typeof responseLog.error === 'string' ? (
                          <Box>
                            <Text 
                              size="1" 
                              weight="medium" 
                            ...
                            <Text 
                              size="1" 
                              className={resolvedTheme === 'dark' ? 'text-red-300' : 'text-red-600'}
                            >
                              {responseLog.error ? String(responseLog.error) : 'Unknown error'}
                            </Text>
                          </Box>
                        ) : null}
                      </Box>
