...
_Complete command-line interface documentation for AI Image Studio_
...
AI Image Studio includes a powerful CLI tool built with Commander.js, providing developers and power users with command-line access to all major features. The CLI supports image generation, project management, configuration, and batch operations.
...
- AI Image Studio project installed
...
```bash
# Basic generation
image-studio generate single --prompt "a beautiful sunset"

# With specific model and size
image-studio generate single \
...
# With details
image-studio project list --detailed

# Filter by category
image-studio project list --category social-media
```
...
# Export as ZIP
image-studio project export --name "My Project" --zip

# Export with assets
image-studio project export --name "My Project" --include-assets
```
...
image-studio project import --directory ./project-folder

# Import with validation
image-studio project import --file project.zip --validate
```
...
image-studio project delete --name "My Project" --force

# Delete with assets
image-studio project delete --name "My Project" --include-assets
```
...
image-studio config init --template production
```
...
image-studio config validate --api-key --endpoints
...
image-studio config set-api-key --key "your-api-key"
...
image-studio config show --api-key --endpoints

# Export configuration
image-studio config show --export ./config-backup.json
```
...
image-studio config reset --api-key --endpoints
...
# List with details
image-studio models list --detailed

# Filter by type
image-studio models list --type image-generation
```
...
image-studio models capabilities --model dalle-3

# Test model connection
image-studio models test --model dalle-3
```
...
image-studio models configure --model dalle-3

# Set model parameters
image-studio models configure \
  --model dalle-3 \
  --max-tokens 1000 \
  --temperature 0.7
```
...
```bash
# Search for assets
image-studio assets search --query "nature landscape"

# Search with filters
image-studio assets search \
...
```bash
# Download by ID
image-studio assets download --id "12345" --output ./assets

# Download multiple assets
image-studio assets download \
...
# Organize assets
image-studio assets organize --directory ./assets

# Clean up unused assets
image-studio assets cleanup --dry-run
```
...
image-studio dev start --port 3001

# Start with hot reload
image-studio dev start --watch

# Start with debug mode
image-studio dev start --debug
```
...
image-studio dev build

# Build with optimization
image-studio dev build --optimize

# Build specific components
image-studio dev build --components cli,api
```
...
```bash
# Run all tests
image-studio dev test

# Run specific test suite
image-studio dev test --suite cli

# Run with coverage
image-studio dev test --coverage
```
...
async function generateImages(prompts) {
  for (const prompt of prompts) {
    const command = `image-studio generate single --prompt "${prompt}" --output ./generated`;
...
Create `~/.image-studio/config.json`:
...
Create custom project templates in `~/.image-studio/templates/`:
...
```bash
# Validate project
image-studio project validate --name "My Project"

# Repair project
image-studio project repair --name "My Project"

# Export for backup
image-studio project export --name "My Project" --backup
```
  ...
  generate:
    ...
    steps:
      ...
      - name: Configure Azure
        run: |
          image-studio config set-api-key --key ${{ secrets.AZURE_API_KEY }}
          image-studio config set-endpoint --endpoint ${{ secrets.AZURE_ENDPOINT }}

      - name: Generate images
        run: |
          image-studio generate batch --file prompts.txt --output ./generated
