...
*Comprehensive troubleshooting guide for Azure Image Studio*
...
This guide helps you diagnose and resolve common issues with Azure Image Studio. Whether you're experiencing problems with image generation, configuration, or performance, this guide provides step-by-step solutions.
...
azure-image-studio dev check-system
...
azure-image-studio dev health-check

# Check specific components
azure-image-studio dev health-check --component azure
azure-image-studio dev health-check --component assets
azure-image-studio dev health-check --component projects
```
...
1. **Check File Permissions:**
   ...
   ls -la ~/.azure-image-studio/
   
   # Fix permissions if needed
   chmod 755 ~/.azure-image-studio/
   ```

2. **Clear Configuration Cache:**
   ```bash
   azure-image-studio config clear-cache
   ```
...
2. **Test API Connection:**
   ```bash
   azure-image-studio assets test-connection
   ```
...
3. **Project Validation:**
   ```bash
   azure-image-studio project validate --name "Project Name"
   ```

4. **Backup and Restore:**
   ```bash
   # Export project
   azure-image-studio project export --name "Project Name" --backup
   
   # Clear project cache
   azure-image-studio project clear-cache
   ```
...
2. **Project Repair:**
   ```bash
   azure-image-studio project repair --name "Project Name"
   ```

3. **Restore from Backup:**
   ```bash
   azure-image-studio project restore --backup ./backup/project.zip
   ```
...
azure-image-studio config show --debug

# Validate with verbose output
azure-image-studio config validate --verbose
```

#### Debug Generation
```bash
# Generate with debug info
azure-image-studio generate single --prompt "test" --verbose --debug
...
azure-image-studio dev check-files
```
...
azure-image-studio dev logs --type error
azure-image-studio dev logs --type generation
...
#### Log Locations
- **Application Logs**: `~/.azure-image-studio/logs/`
- **Error Logs**: `~/.azure-image-studio/logs/error.log`
- **Generation Logs**: `~/.azure-image-studio/logs/generation.log`
- **System Logs**: `~/.azure-image-studio/logs/system.log`

#### Common Log Patterns
```bash
# Search for errors
grep -i "error" ~/.azure-image-studio/logs/*.log

# Search for generation issues
grep -i "generation" ~/.azure-image-studio/logs/*.log

# Search for API errors
grep -i "api" ~/.azure-image-studio/logs/*.log
```
...
azure-image-studio config export --backup

# Reset all settings
azure-image-studio config reset --all

# Reinitialize configuration
azure-image-studio config init

# Restore from backup if needed
azure-image-studio config import --backup ./backup/config.json
```
...
# Export all projects
azure-image-studio project export-all --output ./backup

# Repair specific project
azure-image-studio project repair --name "Project Name"

# Restore from backup
azure-image-studio project import --file ./backup/project.zip
```

### Data Recovery
```bash
# Check for corrupted data
azure-image-studio dev check-data

# Repair corrupted files
azure-image-studio dev repair-data

# Restore from backup
azure-image-studio dev restore-data --backup ./backup
```
